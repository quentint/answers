...
function initSlideShow() {
	...
	if (jQuery('div.slideshow').length > 0) {
		...
		inifixGallery('div.slideshow');
	}
}

// fix width items in gallery
...
function inifixGallery(gal) {
	...
	jQuery(gal).each(function() {
		...
		function fixSize() {
			list.css({
				height: lis.filter('.active').outerHeight()
			});
		}
		fixSize();
		jQuery(window).bind('load resize orientationchange', fixSize);
	});
}
...
jcf.lib = {
	...
	event: (function() {
		...
		function fixEvent(e) {
			...
			e.stopPropagation = e.stopPropagaton ||
			function() {
...
jcf.addModule({
	...
	defaultOptions: {
		...
		// or 'top'
		wrapperClass: 'select-area select-lang',
...
function initInputs() {
	...
	// collect all items
	var inputs = [].concat(
	PlaceholderInput.convertToArray(document.getElementsByTagName('input')), PlaceholderInput.convertToArray(document.getElementsByTagName('textarea')));
	// apply placeholder class on inputs
	for (var i = 0; i < inputs.length; i++) {
		if (inputs[i].className.indexOf('default') < 0) {
			var inputType = PlaceholderInput.getInputType(inputs[i]);
			if ((opt.clearInputs && inputType === 'text') || (opt.clearTextareas && inputType === 'textarea') || (opt.clearPasswords && inputType === 'password')) {
